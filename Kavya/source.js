...
var _Sources = (() => {
  ...
  async function getReadingListDetails(mangaId, requestManager, stateManager) {
    const kavitaAPI = await getKavitaAPI(stateManager);
  ...
  async function getOptions(stateManager) {
    ...
    const showReadingLists = await stateManager.retrieve("showReadingLists") ?? DEFAULT_VALUES.showReadingLists;
  ...
  var Kavya = class {
    ...
    async getMangaDetails(mangaId) {
      const detailsFunction = mangaId.startsWith("rl-") ? getReadingListDetails : getSeriesDetails;
    ...
    async getChapters(mangaId) {
      ...
      if (mangaId.startsWith("rl-")) {
        ...
        for (const rlItem of result) {
          const progress = rlItem.pagesRead === 0 ? "" : rlItem.pagesTotal === rlItem.pagesRead ? "\xB7 Read" : `\xB7 Reading ${rlItem.pagesRead} page`;
          const item = {
            id: `${rlItem.chapterId}`,
    ...
    async getHomePageSections(sectionCallback) {
      ...
      for (const section of sections) {
        let apiPath, body = {}, id = "id", title = "name", isReadingList = false;
        switch (section.id) {
          case "ondeck":
